...
function carousel_cart($db,$cus_id)
{
    ...
    for ($i=0; $i<pg_num_rows($skuid);$i++)
     {	
        ...
        $datas['template']['columns'][$i]['text'] = $namearray[$i][2].' '.$skuarray[$i][3].' 1 ชิ้น';
        $datas['template']['columns'][$i]['actions'][0]['type'] = 'postback';
...
function add_to_order($db,$cus_id,$cart_avail)
{
	...
	$time = date("H:i:sa");
	$date = date("Y-m-d") ;
	pg_query($db,"INSERT INTO order (order_id,cartp_id,total_price,date,time,order_status) VALUES ('$order_id','$cart_avail','$total_price','$date','$time','waiting for payment')");
